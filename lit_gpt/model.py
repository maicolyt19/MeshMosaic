...
class Hourglass(torch.nn.Module):
    ...
    def forward(self, x,  
        ...
        if self.hourglass is not None:
            ...
            if exists(mask):
                # 修改reduce操作以处理[B,1,seq,seq]格式的mask
                # downsampled_mask = reduce(padded_mask, 'b 1 (l t) (n s) -> b 1 l n', 'sum', s = self.k, t=self.k) > 0
...
class GPT(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.norm_global = ZeroNorm(config.n_embd)
        self.linear = nn.Linear(1024, config.n_embd)
        self.zero_linear = ZeroLinear(1024, config.n_embd)
        self.seq_compressor = SeqCompressor(vocab_size=config.padded_vocab_size, emb_dim=64, hidden_dim=128, out_dim=256)
        self.bd_token_proj = nn.Linear(256, config.n_embd)
        self.ablation_gpc = False
        self.ablation_bdt = False
    ...
    def create_sliding_window_attention_mask(self,sequence_length=5000, window_size=1000, start=0, boundry_length=None) -> torch.Tensor:
        if boundry_length is not None:
            batch_size = len(boundry_length)
            mask = torch.zeros((batch_size, sequence_length, sequence_length), dtype=torch.bool)

            for b in range(batch_size):
                for i in range(sequence_length):
                    start_index = max(i - window_size + 1, boundry_length[b])
                    mask[b, i, start_index:i+1] = 1  
            
            # to [b,1,seq_len,seq_len]
            ...
            for i in range(sequence_length):
                start_index = max(i - window_size + 1, start)
    ...
    def forward(
        self, idx: torch.Tensor, pc=None, global_pc=None, bd_token=None, max_seq_length: Optional[int] = None,\
         input_pos: Optional[torch.Tensor] = None,start: Optional[int] = 0,window_size: Optional[int] = 4000, boundry_length: Optional[int] = 0
    ) -> torch.Tensor:
        #cat idx and boundryidx
        # idx = torch.cat([boundryidx,idx], dim=1)
        ...
        if use_kv_cache:
            ...
            if window_size > 0:
               mask= self.create_sliding_window_attention_mask(sequence_length=T,window_size=window_size,start = start).to(idx.device)
            else:
                # build casual mask
        ...
        if not self.ablation_gpc:
            if global_pc is not None:
                global_cond_embeds = self.conditioner(pc = global_pc) #(bs,257,1024)
                global_cond_embeds = self.zero_linear(global_cond_embeds) 
                global_cond_embeds = self.norm_global(global_cond_embeds)
                cond_embeds = torch.cat([cond_embeds, global_cond_embeds], dim=1)
            else:
                global_cond_embeds = None
        
        if not self.ablation_bdt:
            if bd_token is not None:
                bd_token_embeds = self.seq_compressor(bd_token)  # (bs, 256)
                bd_token_embeds = self.bd_token_proj(bd_token_embeds).unsqueeze(1)  # (bs, 1, n_embd)
                cond_embeds = torch.cat([cond_embeds, bd_token_embeds], dim=1)
            else:
                bd_token_embeds = None
...
class CausalSelfAttention(nn.Module):
    ...
    def scaled_dot_product_attention(
        ...
        # # 处理batch-wise的mask
        # if mask is not None and mask.dim() == 5:  # [B, 1, 1, seq_len, seq_len]
        #     # 将mask扩展到所有注意力头，保持batch维度
        #     mask = mask.expand(-1, q.size(1), -1, -1, -1)  # [B, n_heads, 1, seq_len, seq_len]
        #     # 移除多余的维度
        #     mask = mask.squeeze(2)  # [B, n_heads, seq_len, seq_len]
...
#         # Create trainable copy of the original block
#         self.trainable_block = type(original_block)(config)
#         # Copy weights from original block
#         self.trainable_block.load_state_dict(original_block.state_dict())
#         # Freeze original block
#         for param in original_block.parameters():
#             param.requires_grad = False
#         # Add zero linear layer
#         self.zero_linear = ZeroLinear(config.n_embd, config.n_embd)

#     def forward(self, x, *args, **kwargs):
#         # Original block output
#         original_output = self.original_block(x, *args, **kwargs)
#         # Trainable block output
#         trainable_output = self.trainable_block(x, *args, **kwargs)
#         # Add zero linear transformation
#         zero_output = self.zero_linear(x)
#         # Combine outputs
#         return original_output + trainable_output + zero_output

# class ControlNetHourglass(nn.Module):
#     def __init__(self, original_hourglass, config):
#         super().__init__()
#         self.original_hourglass = original_hourglass
#         # Create trainable copies of all components
#         self.trainable_pre_layer = nn.ModuleList([
#             ControlNetBlock(block, config) for block in original_hourglass.pre_layer
#         ])
#         self.trainable_downsample = LinearDownsample(config.n_embd, original_hourglass.k)
#         self.trainable_upsample = LinearUpsample(config.n_embd, original_hourglass.k)
#         if original_hourglass.hourglass is not None:
#             self.trainable_hourglass = ControlNetHourglass(original_hourglass.hourglass, config)
#         self.trainable_post_layer = nn.ModuleList([
#             ControlNetBlock(block, config) for block in original_hourglass.post_layer
#         ])
#         # Add zero linear layers
#         self.zero_linear_pre = ZeroLinear(config.n_embd, config.n_embd)
#         self.zero_linear_post = ZeroLinear(config.n_embd, config.n_embd)

#     def forward(self, x, *args, **kwargs):
#         # Original hourglass output
#         original_output = self.original_hourglass(x, *args, **kwargs)
        
#         # Trainable path
#         x_train = x
#         # Pre-layers
#         for block in self.trainable_pre_layer:
#             x_train = block(x_train, *args, **kwargs)
        
#         # Downsample
#         x_train = self.trainable_downsample(x_train)
        
#         # Recursive hourglass
#         if hasattr(self, 'trainable_hourglass'):
#             x_train = self.trainable_hourglass(x_train, *args, **kwargs)
        
#         # Upsample
#         x_train = self.trainable_upsample(x_train)
        
#         # Post-layers
#         for block in self.trainable_post_layer:
#             x_train = block(x_train, *args, **kwargs)
        
#         # Zero linear transformations
#         zero_output = self.zero_linear_pre(x)
#         zero_output += self.zero_linear_post(x_train)
        
#         # Combine outputs
#         return original_output + x_train + zero_output

# class ControlNetGPT(GPT):
...
#         # Create trainable copies
#         self.trainable_transformer = nn.ModuleDict({
#             'wte': nn.Embedding(config.padded_vocab_size, config.n_embd),
#             'h': ControlNetHourglass(original_model.transformer.h, config),
#             'ln_f': config.norm_class(config.n_embd, eps=config.norm_eps)
#         })
        
#         # Add zero linear layers
#         self.zero_linear_input = ZeroLinear(config.n_embd, config.n_embd)
#         self.zero_linear_output = ZeroLinear(config.n_embd, config.n_embd)

#     def forward(
#         self, idx: torch.Tensor, boundryidx=None, localpc=None, pc=None, max_seq_length: Optional[int] = None,\
...
#         # Trainable path
#         x = self.trainable_transformer.wte(idx)
#         x = self.trainable_transformer.h(x, pc)
#         x = self.trainable_transformer.ln_f(x)
        
#         # Zero linear transformations
#         zero_output = self.zero_linear_input(x)
#         zero_output += self.zero_linear_output(x)
        
#         # Combine outputs
#         return original_output + x + zero_output
