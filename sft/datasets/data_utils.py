...
def process_mesh_xr(vertices, faces, quantization_bits=8, quan = True,augment=True, augment_dict=None):
    """Process mesh vertices and faces."""
...
def augment_mesh(vertices, scale_min=0.95, scale_max=1.05, rotation=1.0, jitter_strength=0.):
    ...
    if rotation != 0:
        ...
        rotation_angles = random.choice(angles)  
        # rotation_axis = random.choice(axes)    

        radian = np.pi / 180.0 * rotation_angles
        
        rotation = Rotation.from_rotvec(radian * np.array(axes[0]))

        vertices = rotation.apply(vertices)
        rotation_angles = random.choice(angles)
        radian = np.pi / 180.0 * rotation_angles
        rotation = Rotation.from_rotvec(radian * np.array(axes[1]))
        vertices = rotation.apply(vertices)
        
        rotation_angles = random.choice(angles)
        radian = np.pi / 180.0 * rotation_angles
        rotation = Rotation.from_rotvec(radian * np.array(axes[2]))
        vertices = rotation.apply(vertices)
